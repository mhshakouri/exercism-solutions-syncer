...
export const DATAREFRENCE = [
  ...
  { color: 'grey', value: 8, multiplier: 100000000 },
  { color: 'white', value: 9, multiplier: 1000000000 }
]
...
export const colorReference = (color: string) => {
  return DATAREFRENCE.find(item => item.color === color)
}

export const colorValue = (color: string) => {
  return colorReference(color)?.value ?? 0
}
...
export const formatValue = (value: number) => {
  return value.toLocaleString('en-US', { maximumFractionDigits: 0 }).split(',')[0]
}

export const formatSuffix = (value: number) => {
  const prefix = PREFIXES[Math.floor(Math.log10(value) / 3)]
  return prefix ? prefix + 'ohms' : 'ohms'
}

export const formatValueWithSuffix = (value: number) => {
  return `${formatValue(value)} ${formatSuffix(value)}`
}

export function decodedResistorValue(colors: string[]) {
  return formatValueWithSuffix(decodedValue(colors) * (colorReference(colors[2])?.multiplier ?? 1))
}
